...
EVT_DEFINE_USER_FUNC(evtTot_CheckPlayerMegatonHit) {
    ...
    auto* self = ttyd::battle::BattleGetUnitPtr(battleWork, self_id);
    auto* target = ttyd::battle::BattleGetUnitPtr(battleWork, id);
    // Check for fully charged damage + a player character as target.
    if (target && self && self->unit_work[3] == 20 &&
        (target->true_kind == BattleUnitType::MARIO ||
        target->true_kind >= BattleUnitType::GOOMBELLA)) {
        AchievementsManager::MarkCompleted(AchievementId::MISC_MEGATON_BOMB);
    }
    return 2;
}
...
LBL(10)
    ...
    IF_EQUAL(LW(5), 1)
        ...
        IF_EQUAL(LW(8), PTR(&unitBombzo_weapon_Megaton))
            USER_FUNC(evtTot_CheckPlayerMegatonHit, LW(3))
        END_IF()
    END_IF()
    LBL(80)
...
EVT_BEGIN(partySandersAttack_NormalAttack)
    ...
    // Player didn't input anything; do light damage by default.
    IF_EQUAL(LW(1), 2)
        USER_FUNC(btlevtcmd_GetPos, -2, LW(0), LW(1), LW(2))
        USER_FUNC(btlevtcmd_FaceDirectionAdd, -2, LW(0), 20)
        USER_FUNC(btlevtcmd_SetMoveSoundDataWork, -2, PTR("SFX_BTL_THUNDERS_ATTACK1"), 0, 0, -1, -1, 0, 0)
...
EVT_BEGIN(partySandersAttack_TimeBombSet)
    ...
    BROTHER_EVT_ID(LW(15))
        ...
        DO(LW(14))
            ...
            ELSE()
                // Throw automatically every 30-60 frames.
                SET(LW(0), 0)
                IF_EQUAL(LW(14), 1)
                    SET(LW(0), 1)
                    USER_FUNC(evt_sub_random, 30, LW(14))
                    ADD(LW(14), 30)
                END_IF()
            END_IF()
...
EVT_BEGIN(partySandersAttack_SuperBombAttack)
    ...
    IF_EQUAL((int32_t)GSW_Battle_DooplissMove, 0)
        ...
        USER_FUNC(btlevtcmd_StartAC, 1)
    ELSE()
        // Hardcode attack duration.
        SET(LW(0), 150)
        // Set output damage directly.
        USER_FUNC(evtTot_GetMoveSelectedLevel, MoveType::BOBBERY_BOBOMBAST, LW(5))
        MUL(LW(5), 2)
        ADD(LW(5), 3)
        USER_FUNC(btlevtcmd_AcSetOutputParam, 2, LW(5))
    END_IF()
