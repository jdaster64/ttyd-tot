...
EVT_DEFINE_USER_FUNC(evtTot_MakeBreathWeapon) {
    ...
    if (move_type == MoveType::FLURRIE_GALE_FORCE) {
        ...
        if (ac_result >= 75) {
            ...
            // Don't want to apply what's effectively a 0-turn freeze.
            if (move_level == 1 && GetSWByte(GSW_Battle_DooplissMove))
                ++weapon->freeze_time;
...
EVT_BEGIN(partyClaudaAttack_BreathAttack)
    ...
    IF_EQUAL((int32_t)GSW_Battle_DooplissMove, 0)
        ...
        END_SWITCH()
    ELSE()
        // Hardcode Thunder Storm's power to 8.
        USER_FUNC(btlevtcmd_AcSetOutputParam, 1, 8)
        // Hardcode Blizzard's output fullness.
        USER_FUNC(btlevtcmd_AcSetOutputParam, 2, 95)
    END_IF()
...
EVT_BEGIN(partyClaudaAttack_PredationAttack)
    ...
    ELSE()
        ...
        MUL(LW(5), 2)
        ADD(LW(5), 1)
        USER_FUNC(btlevtcmd_AcSetOutputParam, 0, LW(5))
    END_IF()
