...
LBL(11)
    ...
    IF_SMALL(LW(0), LW(3))
        ...
        GOTO(99)
    END_IF()
    USER_FUNC(evtTot_SpendPermanentCurrency, 1, LW(3))
    MUL(LW(3), -1)
...
EVT_BEGIN(CosmeticShops_TutorialEvt)
    ...
    SET((int32_t)GSWF_CosmeticShopTutorial, 1)
...
EVT_BEGIN(CosmeticShopA_TalkEvt)
    ...
    IF_EQUAL((int32_t)GSWF_CosmeticShopTutorial, 0)
...
EVT_BEGIN(Npc_I_Talk)
    SET(LW(15), (int32_t)ConversationId::NPC_I)
    RUN_CHILD_EVT(Npc_ConversationDrive_Talk)
    RETURN()
EVT_END()
...
EVT_BEGIN(Npc_BubulbP_NameEntry)
    ...
    USER_FUNC(evt_win_nameent_on, 3)
    INLINE_EVT()
        WAIT_MSEC(300)
