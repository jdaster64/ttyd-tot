...
void SelectChestContents() {
    ...
    int32_t kMoveWeights[] = { 17, 17, 14, 52 };
...
EVT_DEFINE_USER_FUNC(evtTot_AfterItemBought) {
    ...
    if (g_Mod->state_.GetOption(STAT_PERM_ITEMS_BOUGHT) >= 25) {
        AchievementsManager::MarkCompleted(AchievementId::AGG_BUY_ITEMS_25);
    }
...
EVT_DEFINE_USER_FUNC(evtTot_GetStarPieceRolls) {
    int32_t extra_rolls = g_Mod->state_.Rand(3, RNG_STAR_PIECE_CHEST);
