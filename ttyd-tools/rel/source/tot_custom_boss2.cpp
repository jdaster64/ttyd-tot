...
EVT_BEGIN(unitDoopliss_pre_transform_event)
    USER_FUNC(btlevtcmd_StatusWindowOnOff, 0)
    WAIT_FRM(50)
...
EVT_BEGIN(unitDoopliss_transform_event)
    ...
    SWITCH(LW(0))
        CASE_EQUAL(0)
            RUN_CHILD_EVT(unitDoopliss_pre_transform_event)
...
EVT_BEGIN(unitDoopliss_phase_event)
    ...
    IF_EQUAL(LW(0), 0)
        ...
        END_IF()
    ELSE()
        // Otherwise, check for phase 2 / 3 dialogue.
...
EVT_BEGIN(unitDoopliss_battle_entry_event)
    // Nothing special; moved dialogue to before first transformation.
    USER_FUNC(evt_btl_camera_set_mode, 0, 0)
    RETURN()
EVT_END()
